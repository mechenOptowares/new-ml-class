...
base_model = DenseNet121(input_shape=(config.img_width, config.img_height, 3),
                         weights='imagenet',
                         include_top=False, # gives all the layer except the last one
                         pooling='avg')

model = add_new_last_layer(base_model, nb_classes) # make new keras model by taking the input from the base model and
# add another layer of the perceptron

# transfer learning
setup_to_transfer_learn(model, base_model)
# setup_to_transfer_learn = get the model from the keras and makes it un-learnable
